...
type UniformsMap = Record<string, UniformValue>
...
// ─── ShaderMaterial (R3F inner) ───────────────────────────────────────────────

function ShaderMesh({ source, uniforms }: { source: string; uniforms: UniformsMap }) {
  const { size } = useThree()
  ...
  const preparedUniforms = useMemo((): Record<string, THREE.IUniform> => {
    ...
    for (const name in uniforms) {
      ...
      switch (u.type) {
        case 'uniform1f':
          out[name] = { value: u.value as number }
          break
        case 'uniform1i':
          out[name] = { value: u.value as number }
          break
        ...
        case 'uniform3fv':
          ...
          break
        case 'uniform3f':
          out[name] = { value: new THREE.Vector3().fromArray(u.value as number[]) }
          break
        case 'uniform2f':
          out[name] = { value: new THREE.Vector2().fromArray(u.value as number[]) }
          break
      }
    }
...
function DotMatrix({
  ...
  const uniforms = useMemo((): UniformsMap => {
    ...
    if (colors.length === 2) {
      colorsArray = [...Array(3).fill(colors[0]), ...Array(3).fill(colors[1])] as number[][]
